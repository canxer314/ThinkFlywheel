...
- 例外: `type/task`、`type/project`、`type/flow` 由对应命令触发时直接写入
...
| 级别 | 操作范围 | 示例 |
|------|---------|------|
| **AI 自主** | 无需确认，直接执行 | 更新 index.md、追加 log.md、更新 MOC、生成 daily briefing、生成 health report、移动 Sources(已处理)、更新 task 状态/下一步行动、更新 project 进度脉搏、更新 flow 内容/迭代日志 |
...
- 创建任务时，检查 Flows/ 是否有匹配的永久型流程
- 更新任务状态时，同步更新 frontmatter 的 `updated` 字段
- 任务归档时，提醒用户做 /retro
- /retro 后触发三向回流：原子卡片 + flow 经验回写 + project 经验区更新
...
- 任务笔记: 必须包含完成定义、下一步行动(独立于步骤清单)、行动步骤、进度日志、问题与吐槽、复盘与衍生。A-F-O-T 命名
- 专案笔记: 鸟瞰地图 — 专案完成定义、里程碑地图、任务清单可视化、进度评估、专案经验区
- 流程笔记: 永久型任务笔记 — 适用场景、流程目标、标准步骤、易错点、迭代日志
...
| "目标/项目/规划" | `/project` | 创建专案鸟瞰地图，查询历史类似项目 |
| "流程/SOP/常做的事" | `/flow` | 创建永久型任务笔记，从历史任务提炼步骤 |
