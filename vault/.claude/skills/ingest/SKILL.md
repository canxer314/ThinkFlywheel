...
不是你读了一篇文章就完了——而是文章的关键知识被整合进你已有的知识体系：新的概念被提取为原子卡片，与你已有知识的矛盾被标记，与你正在做的任务关联起来。
...
```
Sources/inbox/  →  /ingest  →  Cards/reading/ (阅读摘要)
                    ↓ (自动触发)
                 /note 双提议  →  Cards/atomics/ (原子卡片)
                    ↓
                 /review  →  FSRS-6 间隔重复

活跃任务  ←  知识关联（自动标记 related_tasks）
```
