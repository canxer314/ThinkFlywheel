---
name: decide
description: 结构化决策记录。创建决策日志→查询 vault 历史类似决策作为上下文→FSRS 复查提醒。当用户说"decide"、"decision"、"决策"、"决定"、"选择"、"选哪个"时触发。
invocation: user
arguments:
  - name: title
    description: 决策简述
    required: true
---

# /decide Command

结构化决策记录。不是帮你做决策——而是帮你**追踪决策质量**。好决策和坏结果可以同时存在，没有追踪就分不清是运气还是判断。

## 核心理念

大多数人不记录决策，所以他们从自己的决策历史中学不到任何东西。ThinkFlywheel 把决策当作一类一等公民的知识——它们被记录、被复查、被纳入间隔重复，让你慢慢学会自己的偏见和模式。

## Behavior

### Step 1: 接收决策描述

从用户输入中提取：
- 决策标题
- 触发上下文（什么情况需要做这个决策？）
- 如果有的话，用户已经有哪些候选选项

### Step 2: 查询历史上下文（双通道：Vault + AgentMemory MCP）

在做决策之前，查询 vault 积累的相关经验 + MCP 跨会话历史：

**Vault 通道**：
1. **历史类似决策**: 搜索 `Decisions/` 中 domain 相同、关键词相似的决策——当时选了哪个选项？结果如何？
2. **相关知识**: 搜索 `Cards/` 中 domain 匹配的卡片——有什么概念或洞察可以用上？
3. **相关教训**: 搜索 `Cards/insights/` 中 domain 匹配的教训——之前踩过的坑？
4. **活跃任务**: 搜索 `Tasks/active/` 中可能受此决策影响的任务

**【MCP 通道】跨会话决策上下文召回**：
- `memory_smart_search` 用决策关键词搜索 → 找历史上类似 tradeoff 的讨论（可能未写入 Decisions/）
- `memory_recall` 用 domain + "选" "决定" "权衡" → 找以前做类似选择时的完整讨论上下文
- MCP 可能发现：当时讨论过但最终未选的选项、讨论中提到的隐性约束、事后回顾时发现的问题
- MCP 结果与 vault 记录冲突 → vault 胜出；MCP 补充了 vault 没有的信息 → 标注 "🆕 MCP 发现"

将查询结果（标注来源：vault / MCP / 两者）作为决策上下文展示给用户。

### Step 3: 辅助构建选项（不替用户做决策）

AI 可以：
- 列出常见的决策框架（如利弊分析、决策矩阵）供用户选择
- 指出选项列表中可能的遗漏（"你考虑过 C 选项吗？"）
- 对每个选项提出探测性问题（"如果选 A，最坏情况是什么？"）

AI 不可以：
- 替用户做决策
- 隐藏或弱化某些选项
- 用倾向性语言引导选择

### Step 4: 创建决策记录

在 `Decisions/DEC-{YYYY}-{NNN}.md` 创建：

```markdown
---
type: decision
domain: {domain}
status: made
created: YYYY-MM-DD
updated: YYYY-MM-DD
source: null
related_tasks: [{受影响的任务}]
related_cards: [{相关的知识卡片}]
review_checkpoint: {复查日期}
---

# DEC-{YYYY}-{NNN}: {决策标题}

## Context
{触发此决策的背景和约束}
{AI 从 vault 查询到的历史类似决策、相关知识、教训}

## Options Considered
| 选项 | 优点 | 缺点 | 风险 | 最坏情况 |
|------|------|------|------|---------|
| A: {option} | | | | |
| B: {option} | | | | |
| C: {option} | | | | |

## Choice
**选定**: {用户的选择}

## Rationale
{为什么选这个而不是其他}
- 关键假设:
- 放弃的价值（选择 A 意味着不做 B 和 C）:

## Expected Outcome
{成功是什么样子，什么时候判断这个决策是对的}

## Review Checkpoint
**复查日期**: {YYYY-MM-DD}
**复查问题**:
1. 实际结果符合预期吗？
2. 关键假设还成立吗？
3. 如果今天重新选，会选不同的吗？
```

### Step 5: 注册复查提醒

1. 决策复查日期（默认 30 天后，用户可调整）写入 `review_checkpoint` 字段
2. 创建一个类型为 decision-review 的卡片注册到 FSRS：
   ```
   Q: {决策标题} — 实际结果符合预期吗？
   A: 预期: {Expected Outcome}
   ```

### Step 6: 更新索引（AI 自主）

- 更新 `index.md` 的 "Decisions" 节
- 追加 `log.md`: `## [YYYY-MM-DD] decide | DEC-{YYYY}-{NNN} | {title}`
- 如果用户接受，将决策记录链接到受影响的任务

---

## 决策复查（用户说"复查决策"/"decision review"时）

1. 读取即将到复查日期的决策记录
2. 展示原始决策和预期结果
3. 询问用户: 实际结果如何？关键假设还成立吗？
4. 更新决策 status: `reviewed` 或 `overturned`
5. 如果 overturned: 创建 insight 卡片捕获教训
6. 设置下一次复查日期（如果仍有待观察的假设）

---

## 特殊情况

- **紧急决策** (用户说"没时间走完整流程"): 记录最小版本——选择 + 理由 + 预期，事后补全
- **不可逆决策**: 标记为 `irreversible`，建议更谨慎的选项评估
- **决策事后才发现是错的**: 特别有价值——标记为 `overturned`，创建 insight 卡片深挖原因

## 🚫 不要做什么（反模式）

| # | 反模式 | 为什么不要做 | 正确做法 |
|---|--------|-------------|---------|
| 1 | **替用户做决策** | 决策后果由用户承担，AI 无权代劳 | 只辅助构建选项、探测问题、提供框架，Choice 字段由用户填写 |
| 2 | **隐藏或弱化某些选项** | 选择性呈现=操纵决策 | 所有候选人选项平等展示，客观列优缺点 |
| 3 | **用倾向性语言引导选择** | "A 方案明显更好"=越权 | 只描述事实和风险，不做价值判断 |
| 4 | **跳过历史查询直接创建决策** | 历史类似决策是最有价值的前车之鉴 | Step 2 必须查询 Decisiions/、Cards/insights/、Cards/、Tasks/ + MCP 跨会话召回 |
| 5 | **不设置 review_checkpoint** | 没有复查节点的决策=永远不会被复盘 | Step 5 必须设置复查日期（默认 30 天）+ 注册 FSRS 提醒卡片 |
| 6 | **只用 vault 不用 MCP 查历史** | 很多决策讨论发生在会话中，未写入 Decisions/——MCP 是唯一能召回这些上下文的通道 | Step 2 必须双通道（vault + MCP），MCP 不可用时才降级 |

## 失败处理

| 场景 | 处理 |
|------|------|
| **历史查询无结果**（vault 中无类似决策/知识） | 告知用户"这是该领域首个记录的决策"，正常创建决策记录 |
| **Decisions/ 目录不存在** | 创建目录；失败则告知"vault 目录结构异常，建议运行 /health" |
| **编号冲突**（DEC-YYYY-NNN 已存在） | 自动递增 NNN 至下一个可用编号，告知用户编号变化 |
| **FSRS 复查提醒注册失败** | 不阻塞决策创建；复查日期仍写入 review_checkpoint 字段，手动追踪 |
| **用户拒绝提供 review_checkpoint 日期** | 默认 30 天后，标注"⚠️ 复查日期未确认，建议后续补设" |
| **AgentMemory MCP 不可用** | 跳过 MCP 通道，仅用 vault 查询历史上下文；在决策记录中标注"ℹ️ MCP 不可用，历史上下文仅基于 vault" |
