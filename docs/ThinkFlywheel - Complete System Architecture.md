...
**Core Philosophy:** Every task you complete should make every future task easier. Knowledge without action is dead; action without knowledge is blind; both without memory are wasted. This is not a knowledge management system or a task manager — it is a personal compound interest engine for your cognition.
...
```mermaid
flowchart TD
    L4["L4: GOVERNANCE<br/>/health /decide /briefing<br/>AI scans: broken links, orphans,<br/>knowledge-action gaps, contradictions"]
    L3["L3: TASK — 防弹笔记法<br/>/task /project /retro /flow /meeting<br/>'What to do'"]
    L2["L2: KNOWLEDGE — LLM Wiki<br/>/note /ingest /query<br/>'What I know'"]
    L1["L1: MEMORY — FSRS-6 SR<br/>/review fsrs_engine.py<br/>'What I remember'"]
...
Key insight: This is NOT a stack where higher layers are more important. It is a **flywheel** where each layer strengthens the others. A task completed generates knowledge (/retro —> /note); knowledge enters the memory layer (/review); memorized knowledge surfaces during new tasks (/briefing); better-executed tasks generate better knowledge.
...
| FSRS/Atomic Notes | L1 (Memory) | fsrs_engine.py; /review using FSRS-6; SR cards from task insights |
| Agent-PMO (simplified) | L3+L4 | /project replaces /initiate+/plan+/monitor; /health adapts /lint+/query |
...
### 3.5 Frontmatter Standard

All cards use this YAML structure:
...
### 4.1 Overview: 12 skills in 5 groups
...
| Time | Skill | Purpose |
|------|-------|---------|
| Morning | `/briefing` | Daily anchor — the only mandatory daily run |
| Day | `/task` | Create task when new work arrives |
| Day | `/ingest` | Process articles into reading summaries |
| Day | `/note` | Extract knowledge from dialogue |
| Evening | `/retro` | Retrospect completed tasks |
| Evening | `/review` | 5-10 min spaced repetition review |
| Weekend | `/health full` | Full system health check |
...
**Key modification from knowledge-mgmt:** Added automatic related_tasks tagging. When /note is called during a task session, created cards automatically link back to the task. This is the "task anchor" that knowledge-mgmt is missing.
...
**Why this exists:** This fills the GAP that both knowledge-mgmt and agent-pmo have. knowledge-mgmt has NOTE but no TASK completion trigger. agent-pmo has /close but NO knowledge extraction. This skill bridges them. The 3-way reflux solves the core defect of almost all task systems: experience stays trapped inside individual tasks, never reusable across tasks.
...
### 6.1 System Scope — 12 skills, not 15

**Context:** All 10 reports agreed agent-pmo's 15 skills are too many for personal use. The disagreement was whether to keep 4, 6, or 8 skills.

**Decision:** 12 skills total in 5 groups. Rationale:
...
### 6.2 Starting Point — Bulletproof Notes First

**Context:** infrastructure-first (DeepSeek) vs. Bulletproof-first (Claude, Kimi, ChatGPT)
...
### 6.3 AI Autonomy — Dual-Proposal with Reduced Friction

**Context:** full auto (Karpathy) vs mandatory confirmation (knowledge-mgmt) vs middle ground
...
### 6.4 The Spaced Repetition Gap — Must Include

**Context:** Some reports wanted SR integrated, others thought wiki queries could replace it.
...
### 6.5 Folder Structure — English with Chinese Namespaces

**Context:** agent-pmo uses Chinese directories; knowledge-mgmt uses English.
...
### 6.6 /task as Additional Write Channel

**Context:** knowledge-mgmt mandates /note as the ONLY write channel. Some reports suggested /task should also write.
...
### 7.1 Bulletproof Notes (Week 1-2)
...
### 7.2 Knowledge Pipeline (Week 3-4)
...
### 7.4 Project Layer + Bidirectional Flow (Week 7-8)
...
### 7.5 Governance + Refinement (Week 9-12)
...
**State file:** `.obsidian/review_state.json`
```
...
The 10 rule files in `vault/.claude/rules/` auto-load as ground truth at session start. The AI doesn't need to manually read them — they are always in context.
...
|-------|------------|--------|
| ThinkFlywheel | Human external brain — "What I know, do, and remember" | Obsidian vault (Markdown + Git) |
| planning-with-files | Agent RAM — "What step am I on, what's next" | .planning/ directory |
| agentmemory | Agent long-term memory — "What did we discuss last time" | MCP plugin persistence |
